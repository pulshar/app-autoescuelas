...
export default function AdminDashboard({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Quick Alert if there are classes pending review */}
      {(stats?.pending_reviews ?? 0) > 0 && (
        <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-slate-900 text-sm sm:text-base">
                  {stats?.pending_reviews} {stats?.pending_reviews === 1 ? 'clase finalizada pendiente' : 'clases finalizadas pendientes'} de revisión
                </span>
                {/* <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 border border-amber-200 text-amber-800">
                  Acción requerida
                </span> */}
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                Valida si se impartieron con éxito o marca no presentado antes de que se autocierren a las 72 horas.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg bg-amber-100 hover:bg-amber-200 text-amber-800 border border-amber-300 text-xs font-semibold shadow-xs transition-colors shrink-0"
          >
            Revisar ahora ({stats?.pending_reviews})
